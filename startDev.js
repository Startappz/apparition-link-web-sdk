...
async function getDevConfig() {
  ...
  if (await fileExists(devConfigPath)) {
    ...
    await fs.writeFile(devConfigPath, JSON.stringify(defaultDev));
    return defaultDev;
  }
}
...
function executeBuild(config) {
  ...
  exec(`make`, (error, stdout, stderr) => {
    ...
    console.log(
      `Dev websdk build successful, ${DEV_HTML} built from ${TEMPLATE_FILE}`,
    );
    const app = new Koa();
    app.use(serve("."));
    app.listen(config.port, () => {
      ...
      console.log(
        `Example page running at http://localhost:${config.port}/${DEV_HTML}. To edit, update ${TEMPLATE_FILE}`,
      );
    });
  });
}
