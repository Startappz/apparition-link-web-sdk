...
session.get = function (storage, first) {
  var sessionString = first ? "apparition_session_first" : "apparition_session";
  try {
...
session.set = function (storage, data, first) {
  ...
  storage.set("apparition_session", goog.json.serialize(data));
  if (first) {
    storage.set("apparition_session_first", goog.json.serialize(data), true);
  }
};
...
session.update = function (storage, newData) {
  ...
  storage.set("apparition_session", data);
};
...
session.patch = function (storage, data, updateLocalStorage, removeNull) {
  ...
  var session = storage.get("apparition_session", false) || {};
  storage.set("apparition_session", goog.json.serialize(merge(session, data)));

  if (updateLocalStorage) {
    var sessionFirst = storage.get("apparition_session_first", true) || {};
    storage.set(
      "apparition_session_first",
