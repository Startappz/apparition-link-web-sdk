...
journeys_utils.cssRe =
  /<style type="text\/css" id="apparition-css">((.|\s)*?)<\/style>/;
journeys_utils.iframeCssRe =
  /<style type="text\/css" id="apparition-iframe-css">((.|\s)*?)<\/style>/;
journeys_utils.spacerRe = /#apparition-banner-spacer {((.|\s)*?)}/;
journeys_utils.findMarginRe = /margin-bottom: (.*?);/;

journeys_utils.apparition = null;
...
// holds data from Journey that is currently being viewed & data from setApparitionViewData()
...
journeys_utils.getJsAndAddToParent = function (html) {
  ...
  if (match) {
    ...
    script.id = "apparition-journey-cta";
...
journeys_utils.addHtmlToIframe = function (iframe, html, userAgent) {
  ...
  if (metaTag && metaTag.content === "wcag") {
    ...
    scriptTag.text = `
			var focusableElements = 'button, [href], input, select, textarea, [role="button"], h1, [role="text"], .apparition-banner-content';
			var modal = document.getElementById('apparition-banner');
...
function generateIframeOuterCSS(metadata) {
  ...
  if (document.getElementById("apparition-banner-iframe")) {
    document.getElementById("apparition-banner-iframe").style.transition = "";
  }
  ...
  if (!journeys_utils.entryAnimationDisabled) {
    bodyWebkitTransitionStyle =
      ...
      (journeys_utils.animationSpeed * 1.5) / 1000 +
      "s ease; }\n";
    document.body.style.transition =
      "all 0" + (journeys_utils.animationSpeed * 1.5) / 1000 + "s ease";
  ...
  if (journeys_utils.isDesktopJourney) {
    ...
    css +=
      "#apparition-banner-iframe-embed { z-index: 99999!important; height: " +
      bannerHeight +
      "; width: " +
      bannerWidth +
      ...
      "; }\n";
    css +=
      "#apparition-banner-iframe { box-shadow: 0 0 5px rgba(0, 0, 0, .35); width: 1px; min-width: 100%; left: 0; right: 0; border: 0; height: 100%!important; width: 100%!important; " +
      iFrameAnimationStyle +
      "; position: " +
      sticky +
      "; }\n";
    ...
    css +=
      ...
      (journeys_utils.isFullPage
        ...
        : journeys_utils.bannerHeight) +
      "; }\n" +
      "#apparition-banner-iframe { height: " +
      (journeys_utils.isFullPage
        ? journeys_utils.windowWidth + "px"
        : journeys_utils.bannerHeight) +
      "; }";
...
journeys_utils.addIframeInnerCSS = function (iframe, innerCSS) {
  ...
  if (
    ...
    var dismissBackground = doc.getElementsByClassName(
      "apparition-banner-dismiss-background",
    )[0];
    var content = doc.getElementsByClassName("apparition-banner-content")[0];
  ...
  try {
    // get computed background-color of .apparition-banner-content
    var content = doc.getElementsByClassName("apparition-banner-content")[0];
...
journeys_utils.addDynamicCtaText = function (iframe, ctaText) {
  var doc = iframe.contentWindow.document;
  if (doc && doc.getElementById("apparition-mobile-action")) {
    var mobileAction = doc.getElementById("apparition-mobile-action");
...
journeys_utils.animateBannerEntrance = function (banner, cssIframeContainer) {
  ...
  if (journeys_utils.isFullPage && journeys_utils.sticky === "fixed") {
    ...
    bodyCSS.innerHTML = ".apparition-banner-no-scroll {overflow: hidden;}";
    document.head.appendChild(bodyCSS);
    banner_utils.addClass(document.body, "apparition-banner-no-scroll");
  }
  ...
  function onAnimationEnd() {
    ...
    journeys_utils.apparition._publishEvent(
      "didShowJourney",
      journeys_utils.journeyLinkData,
    );
...
journeys_utils._resetJourneysBannerPosition = function (
  ...
  var bannerIFrame = document.getElementById("apparition-banner-iframe");
...
journeys_utils.finalHookups = function (
  ...
  var actionEls = doc.querySelectorAll("#apparition-mobile-action");
...
journeys_utils._getDismissRequestData = function (
  ...
  var dismissRequestData = apparition_view._getPageviewRequestData(
    journeys_utils._getPageviewMetadata(null, metadata),
    null,
    journeys_utils.apparition,
    true,
  );
...
journeys_utils._handleJourneyDismiss = function (
  ...
  if (!testModeEnabled) {
    ...
    var listener = function () {
      journeys_utils.apparition.removeListener(listener);
      ...
      journeys_utils.apparition._api(
        ...
        function (err, data) {
          if (!err && metadata && metadata["dismissRedirect"]) {
            ...
            if (apparition_view.shouldDisplayJourney(data, null, false)) {
              apparition_view.displayJourney(
                data["template"],
                requestData,
                requestData["apparition_view_id"] ||
                  data["event_data"]["apparition_view_data"]["id"],
                data["event_data"]["apparition_view_data"],
                false,
    ...
    journeys_utils.apparition.addListener(
      "apparition_internal_event_didCloseJourney",
      listener,
    );
  }
};
...
journeys_utils.animateBannerExit = function (
  ...
  if (
    ...
    var iFrameOutterCSSBackup =
      document.getElementById("apparition-iframe-css").innerHTML + "\n";
    iFrameOutterCSSBackup +=
      "body { -webkit-transition: all " +
      (journeys_utils.animationSpeed * 1.5) / 1000 +
      "s ease; }\n";
    iFrameOutterCSSBackup +=
      "#apparition-banner-iframe { -webkit-transition: all " +
      journeys_utils.animationSpeed / 1000 +
      "s ease; }\n";
    // in order for updated styles to get applied, we have to remove all apparition-iframe-css styles
    document.getElementById("apparition-iframe-css").innerHTML = "";
  ...
  setTimeout(function () {
    // remove banner, apparition-css, and apparition-iframe-css
    ...
    banner_utils.removeElement(
      document.getElementById("apparition-iframe-css"),
    );
    banner_utils.removeElement(
      document.getElementById("apparition-journey-cta"),
    );

    // remove margin from all elements with apparition injection div
    if (
      (!journeys_utils.exitAnimationDisabled ||
    ...
    banner_utils.removeClass(document.body, "apparition-banner-no-scroll");
...
journeys_utils.setJourneyLinkData = function (linkData) {
  var data = { banner_id: journeys_utils.apparitionViewId };
  if (
    linkData &&
...
journeys_utils.getValueForKeyInApparitionViewData = function (key) {
  if (!journeys_utils) {
    return false;
  }

  if (!journeys_utils.apparition) {
    return false;
  }

  if (!journeys_utils.apparition._apparitionViewData) {
    return false;
  }

  if (!journeys_utils.apparition._apparitionViewData.data) {
    return false;
  }

  return journeys_utils.apparition._apparitionViewData.data[key];
};

journeys_utils.hasJourneyCtaLink = function () {
  if (!journeys_utils.getValueForKeyInApparitionViewData("$journeys_cta")) {
    return false;
  }

  return (
    journeys_utils.getApparitionViewDataItemOrUndefined("$journeys_cta")
      .length > 0
  );
};

journeys_utils.getApparitionViewDataItemOrUndefined = function (name) {
  if (journeys_utils.getValueForKeyInApparitionViewData(name)) {
    return journeys_utils.apparition._apparitionViewData.data[name];
  }
  return undefined;
};

journeys_utils.getJourneyCtaLink = function () {
  return journeys_utils.getApparitionViewDataItemOrUndefined("$journeys_cta");
};
...
journeys_utils.trySetJourneyUrls = function (
  ...
  var assignUrls = function (data) {
    return urls.reduce((value, url) => {
      ...
      var entry = journeys_utils.getApparitionViewDataItemOrUndefined(url);
      if (entry) {
        value[url] = entry;
      }
      return value;
